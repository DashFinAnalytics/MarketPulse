...
def run_sma_crossover(symbol: str, period: str = '2y',
    ...
    for idx, row in df.iterrows():
        ...
        if row['signal'] == 1 and not in_trade:        # BUY
            ...
            trades.append({
                ...
                'pnl': pnl, 'pnl_pct': (px - entry_px) / entry_px * 100
            })
            shares = 0.0
            in_trade = False
    ...
    if in_trade:
        ...
        capital += proceeds
        trades.append({
            'entry': entry_px, 'exit': last_px,
            'pnl': proceeds - shares * entry_px,
            'pnl_pct': (last_px - entry_px) / entry_px * 100
        })

    return _stats(trades, equity, initial_capital, df,
                  f"SMA Crossover ({fast}/{slow})", symbol)


def run_rsi_strategy(symbol: str, period: str = '2y',
                      rsi_period: int = 14,
    ...
    return _stats(trades, equity, initial_capital, df,
                  f"RSI ({rsi_period}) Mean Reversion", symbol)


def run_bollinger_band_strategy(symbol: str, period: str = '2y',
                                 bb_period: int = 20, std_mult: float = 2.0,
                                 initial_capital: float = 10_000,
                                 position_size_pct: float = 100) -> dict:
    """
    ...
    return _stats(trades, equity, initial_capital, df,
                  f"Bollinger Bands ({bb_period}, {std_mult}σ)", symbol)


def run_buy_and_hold(symbol: str, period: str = '2y',
...
def run_monte_carlo(returns_series, initial_capital=10_000,
