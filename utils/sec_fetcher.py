        ...
        for i, form in enumerate(forms):
            if form in form_types:
                ...
                url  = (f'https://www.sec.gov/Archives/edgar/data/'
                        f'{int(cik_raw)}/{accn}/{doc}') if accn and doc else ''
                results.append({
                    'company':    entity_name,
                    'ticker':     ticker.upper(),
                    'form_type':  form,
                    ...
                    'url':        url or f'https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK={cik_raw}&type={form}'
                })
    ...
    try:
        ...
        for i, form in enumerate(forms):
            if form == '4':
                ...
                url  = (f'https://www.sec.gov/Archives/edgar/data/'
                        f'{int(cik_raw)}/{accn.replace("-","")}/') if accn else ''
                results.append({
                    'ticker':      ticker.upper(),
                    'form_type':   '4',
                    'filed_date':  date,
                    'accession':   accn,
                    'edgar_url':   url or f'https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK={cik_raw}&type=4'
                })
                if len(results) >= limit:
                    break
        return results
