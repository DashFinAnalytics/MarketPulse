...
def create_yield_curve_chart(yields_dict=None):
    ...
    try:
        ...
        symbols = {"3M": "^IRX", "2Y": "^UST2Y", "5Y": "^FVX", "10Y": "^TNX", "30Y": "^TYX"}
        live_yields = {}
...
def create_enhanced_price_chart(symbol, interval_key, use_yfinance=True):
    try:
        ...
        return create_price_chart(
            symbol=symbol, title=title, period=params["period"], interval=params["interval"]
        )
    except Exception as e:
...
def create_forex_heatmap(forex_data):
    ...
    try:
        ...
        clean_labels = [
            p.replace("=X", "").replace("USD", "/USD").replace("EUR", "EUR/") for p in pairs
        ]
...
def create_technical_analysis_chart(
    ...
    try:
        ...
        if show_rsi:
            ...
            fig.add_hline(
                y=30, line_dash="dash", line_color="green", opacity=0.5, row=cur_row, col=1
            )
        ...
        if show_macd:
            ...
            fig.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.5, row=cur_row, col=1)
...
def create_portfolio_allocation_chart(holdings_data):
    ...
    try:
        ...
        fig.update_layout(title="Portfolio Allocation", height=400, template="plotly_white")
        return fig
    except Exception as e:
    ...
    try:
        ...
        for sym in symbols:
            try:
                ...
                fig.add_trace(go.Scatter(x=h.index, y=norm, mode="lines", name=sym))
            except Exception:
                continue
        fig.add_hline(y=100, line_dash="dash", line_color="gray", opacity=0.5)
        fig.update_layout(
            title=f"Holding Performance (normalised, {period})",
...
def create_economic_dashboard_chart(eco_data):
    ...
    try:
        ...
        fig.update_layout(
            title="Economic Indicators — 1-Day % Change",
            xaxis_title="% Change",
            template="plotly_white",
            height=420,
            showlegend=False,
...
def create_equity_curve_chart(equity_curve, initial_capital, strategy_name, benchmark_curve=None):
...
def create_monte_carlo_chart(mc_result, initial_capital):
    ...
    try:
        ...
        # Percentile lines
        for pct, label, color in [(10, "P10", "red"), (50, "Median", "blue"), (90, "P90", "green")]:
            pct_path = np.percentile(paths, pct, axis=0)
            fig.add_trace(
                go.Scatter(
                    x=x, y=pct_path, mode="lines", name=label, line=dict(color=color, width=2)
                )
            )
        fig.add_hline(
            y=initial_capital,
    ...
    try:
        ...
        fig = go.Figure(
            go.Bar(
                ...
                y=scores,
                marker_color=[
                    "green" if sc > 0 else ("red" if sc < 0 else "gray") for sc in scores
                ],
        ...
        fig.add_hline(
            y=30, line_dash="dash", line_color="green", opacity=0.5, annotation_text="Strong UP"
        )
        fig.add_hline(
            y=-30, line_dash="dash", line_color="red", opacity=0.5, annotation_text="Strong DOWN"
        )
...
def create_portfolio_optimization_chart(weights, symbols, method_name):
    ...
    try:
        ...
        fig = go.Figure(
            go.Pie(
                labels=symbols,
                values=[max(0, w) for w in weights],
                hole=0.4,
                textinfo="label+percent",
        ...
        fig.update_layout(
            title=f"Optimal Weights — {method_name}", height=420, template="plotly_white"
        )
        return fig
    except Exception as e:
