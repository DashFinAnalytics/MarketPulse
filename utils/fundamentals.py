        ...
        try:
            ...
            if balance_sheet is not None:
                ...
                if "Total Liabilities Net Minority Interest" in balance_sheet.index:
                    metrics["total_liabilities"] = balance_sheet.loc[
                ...
                if "Stockholders Equity" in balance_sheet.index:
                    metrics["stockholders_equity"] = balance_sheet.loc[
                        "Stockholders Equity"
                    ].tolist()
    ...
    def calculate_growth_rates(self, metrics):
        ...
        try:
            ...
            if "net_income" in metrics and len(metrics["net_income"]) > 1:
                net_income = [
                    float(ni) for ni in metrics["net_income"] if ni is not None and pd.notna(ni)
                ]
                if len(net_income) > 1:
                    net_income.reverse()
                    yoy_growth = [
                        (net_income[i] - net_income[i - 1]) / abs(net_income[i - 1]) * 100
