...
class DatabaseManager:
    ...
    def get_historical_data(self, symbol: str, hours: int = 24) -> List[Dict]:
        ...
        try:
            ...
            records = (
                ...
                .filter(FinancialData.symbol == symbol, FinancialData.timestamp >= cutoff_time)
    ...
    def save_user_preferences(self, user_id: str, preferences: Dict):
        ...
        try:
            ...
            existing = (
                session.query(UserPreferences).filter(UserPreferences.user_id == user_id).first()
            )
            ...
            if existing:
                ...
                user_prefs = UserPreferences(user_id=user_id, **preferences)
    ...
    def create_market_alert(self, user_id: str, symbol: str, alert_type: str, target_price: float):
        ...
        try:
            ...
            alert = MarketAlerts(
                user_id=user_id, symbol=symbol, alert_type=alert_type, target_price=target_price
            )
    ...
    def deactivate_alert(self, alert_id: str):
        ...
        try:
            ...
            alert = (
                session.query(MarketAlerts).filter(MarketAlerts.id == uuid.UUID(alert_id)).first()
            )

            if alert is not None and not isinstance(alert.is_active, Column):
                alert.is_active = False
                session.commit()
            elif alert is not None:
                # Workaround if is_active is a Column, set via setattr
    ...
    def get_market_statistics(self) -> Dict[str, object]:
    ...
    def create_portfolio(
        self, user_id: str, name: str, description: str = "", cash_balance: float = 0.0
    ) -> Optional[str]:
        ...
        try:
            ...
            portfolio = Portfolio(
                user_id=user_id, name=name, description=description, cash_balance=cash_balance
            )
    ...
    def add_holding(
        self, portfolio_id: str, symbol: str, quantity: float, price: float, notes: str = ""
    ) -> bool:
        ...
        try:
            ...
            if existing_holding:
                ...
                setattr(existing_holding, "quantity", float(total_quantity))
                setattr(existing_holding, "average_cost", float(new_average_cost))
                if not isinstance(type(existing_holding).updated_at, Column):
                    setattr(existing_holding, "updated_at", datetime.utcnow())
                else:
                    setattr(existing_holding, "updated_at", datetime.utcnow())
                if notes:
                    setattr(existing_holding, "notes", notes)
            else:
                # Create new holding
    ...
    def sell_holding(
        ...
        try:
            ...
            if holding.quantity < quantity:
                logger.error(
                    f"Insufficient shares: trying to sell {quantity}, only have {holding.quantity}"
                )
                session.close()
                return False

            # Update holding
            setattr(holding, "quantity", float(getattr(holding, "quantity")) - float(quantity))
            setattr(holding, "updated_at", datetime.utcnow())
    ...
    def store_fundamental_analysis(
        self, symbol: str, analysis_type: str, analysis_result: Dict, period: str
    ) -> bool:
    ...
    def get_fundamental_analysis(
        self, symbol: str, analysis_type: Optional[str] = None, limit: int = 5
    ) -> List[Dict]:
