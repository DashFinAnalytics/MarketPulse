...
def create_metrics_comparison_chart(metrics):
    ...
    try:
        ...
        # Prepare data
        data_dict: dict[str, list[str] | list[float]] = {"Date": [str(d) for d in reversed(dates)]}

        if "revenue" in metrics and metrics["revenue"]:
            data_dict["Revenue"] = [
                float(v / 1e9) if v is not None else 0.0 for v in reversed(metrics["revenue"])
            ]
        if "net_income" in metrics and metrics["net_income"]:
            data_dict["Net Income"] = [
                float(v / 1e9) if v is not None else 0.0 for v in reversed(metrics["net_income"])
            ]
        if "operating_income" in metrics and metrics["operating_income"]:
            data_dict["Operating Income"] = [
                float(v / 1e9) if v is not None else 0.0
                for v in reversed(metrics["operating_income"])
            ]
        ...
        for col in df.columns:
            if col != "Date":
                fig.add_trace(go.Bar(name=col, x=df["Date"], y=df[col]))
...
def render_fundamental_analysis_page():
    ...
    if not metrics:
        st.error(
            f"Unable to fetch fundamental data for {stock_symbol}. "
    ...
    with chart_col2:
        if "operating_income" in metrics:
            oi_chart = create_earnings_trend_chart(
                metrics, "operating_income", "Operating Income Trend"
            )
            if oi_chart:
    ...
    with analysis_col1:
        ...
        cached_analyses = db_manager.get_fundamental_analysis(
            stock_symbol, valuation_model, limit=1
        )
        if cached_analyses and len(cached_analyses) > 0:
            cached_at = cached_analyses[0]["created_at"].strftime("%Y-%m-%d %H:%M")
            st.caption(f"💾 Cached analysis available from {cached_at}")
        ...
        if run_analysis or use_cached:
            if use_cached and cached_analyses:
                ...
                with st.spinner(f"Running {valuation_model} analysis with AI..."):
                    analysis_result = ai_valuation_analyzer.analyze_fundamentals(
                        metrics, valuation_model
                    )

                if analysis_result and "error" not in analysis_result:
                    # Store in database
                    db_manager.store_fundamental_analysis(
                        stock_symbol, valuation_model, analysis_result, period
                    )
            ...
            st.info(
                "👈 Select an analysis framework and click 'Run AI Analysis' "
    ...
    if growth_rates:
        ...
        with growth_col1:
            ...
            if "revenue_yoy_growth" in growth_rates:
                avg_yoy = sum(growth_rates["revenue_yoy_growth"]) / len(
                    growth_rates["revenue_yoy_growth"]
                )
        ...
        with growth_col2:
            if "profit_margins" in growth_rates:
                latest_margin = (
                    growth_rates["profit_margins"][0] if growth_rates["profit_margins"] else 0
                )
...
def display_dcf_analysis(result):
    ...
    with col2:
        ...
        st.metric(
            "DCF Fair Value",
            f"${fair_value}" if isinstance(fair_value, (int, float)) else fair_value,
        )
    ...
    if "sensitivity_analysis" in result:
        ...
        with sens_col1:
            ...
            st.metric(
                "Conservative",
                f"${conservative}" if isinstance(conservative, (int, float)) else conservative,
            )
        with sens_col2:
        ...
        with sens_col3:
            ...
            st.metric(
                "Optimistic",
                f"${optimistic}" if isinstance(optimistic, (int, float)) else optimistic,
            )
